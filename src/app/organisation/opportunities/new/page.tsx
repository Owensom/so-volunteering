          ...
          __html: `
            (function () {
              ...
              function bindProgressControls() {
                ...
                var controls = form.querySelectorAll('input, textarea, select, [data-role-progress-control]');
                var events = ['input', 'change', 'keyup', 'blur', 'paste', 'click', 'touchend'];

                controls.forEach(function (control) {
                  events.forEach(function (eventName) {
                    control.addEventListener(eventName, scheduleRoleProgressUpdate, { passive: true });
                  });
                });

                form.addEventListener('input', scheduleRoleProgressUpdate, true);
                form.addEventListener('change', scheduleRoleProgressUpdate, true);
                form.addEventListener('keyup', scheduleRoleProgressUpdate, true);
                form.addEventListener('blur', scheduleRoleProgressUpdate, true);
                form.addEventListener('paste', scheduleRoleProgressUpdate, true);
                form.addEventListener('click', scheduleRoleProgressUpdate, true);
                form.addEventListener('touchend', scheduleRoleProgressUpdate, true);
              }

              function initialiseRoleProgress() {
                bindProgressControls();
                scheduleRoleProgressUpdate();
                window.setTimeout(updateRoleProgress, 350);
                window.setTimeout(updateRoleProgress, 700);
              }

              if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', initialiseRoleProgress);
              } else {
                initialiseRoleProgress();
              }
